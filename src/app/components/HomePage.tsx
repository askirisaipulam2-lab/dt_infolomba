...
import Autoplay from "embla-carousel-autoplay"; // 1. Import plugin autoplay
...
export function HomePage({ competitions, onCompetitionClick }: Props) {
  ...
  // Carousel 1: Free Competitions (Horizontal Scroll biasa)
  const [emblaRef, emblaApi] = useEmblaCarousel({
    loop: false,
    align: "start",
  });

  // Carousel 2: Hero Slider (Loop + Autoplay selama 4 detik)
  const [sliderRef, sliderApi] = useEmblaCarousel(
    ...
    [
      Autoplay({ 
        delay: 4000, 
        stopOnInteraction: false, // Tetap jalan meskipun user klik/drag slider
        stopOnMouseEnter: true    // Berhenti sementara saat kursor mouse berada di atas banner
      })
    ]
  );

  // Sinkronisasi Indikator Titik dengan Embla Slider
  const onSelect = useCallback(() => {
    if (!sliderApi) return;
    setSliderIndex(sliderApi.selectedScrollSnap());
  }, [sliderApi]);
  ...
  // Navigasi manual tombol kiri/kanan Hero Banner
  const sliderPrev = useCallback(() => sliderApi?.scrollPrev(), [sliderApi]);
  const sliderNext = useCallback(() => sliderApi?.scrollNext(), [sliderApi]);
  ...
  const filtered = competitions
    .filter((c) => {
      ...
      // Ditambahkan fallback array kosong jika c.tags bernilai undefined
      const matchQuery = 
  ...
  // Ambil maksimal 5 data saja untuk indikator titik (sesuai slice di render)
  const totalSlides = Math.min(competitions.length, 5);

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-10">
      {/* Hero Slider */}
      <section>
        <div className="relative rounded-3xl overflow-hidden h-64 sm:h-96 lg:h-[28rem] bg-muted group">
          <div ref={sliderRef} className="overflow-hidden h-full cursor-grab active:cursor-grabbing">
            <div className="flex h-full">
              {competitions.slice(0, 5).map((comp) => (
                <div key={comp.id} className="flex-none w-full h-full relative" onClick={() => onCompetitionClick(comp)}>
                  <img src={comp.poster} alt={comp.title} className="w-full h-full object-cover" />
          ...
          {/* Teks utama */}
          <div className="absolute inset-0 flex items-center px-8 sm:px-12 pointer-events-none">
            <div className="max-w-lg text-white space-y-3">
              <div className="flex items-center gap-2">
                <Trophy className="w-5 h-5 sm:w-6 sm:h-6 text-yellow-300" />
                <span className="text-xs sm:text-sm opacity-90">Platform Lomba Mahasiswa Indonesia</span>
              </div>
              <h1 className="text-2xl sm:text-4xl font-bold leading-tight">Temukan Kompetisi Terbaikmu</h1>
              <p className="text-xs sm:text-base text-white/85 leading-relaxed">Ratusan lomba aktif, filter gratis/berbayar, dan lacak deadline — semua dalam satu platform.</p>
            </div>
          </div>

          {/* Slider Controls */}
          <button onClick={sliderPrev} className="absolute left-4 top-1/2 -translate-y-1/2 z-20 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-white/30 hover:bg-white/50 flex items-center justify-center transition-colors backdrop-blur-sm">
            <ChevronLeft className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
          </button>
          <button onClick={sliderNext} className="absolute right-4 top-1/2 -translate-y-1/2 z-20 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-white/30 hover:bg-white/50 flex items-center justify-center transition-colors backdrop-blur-sm">
          ...
          <div className="absolute bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-20 flex gap-2">
            {Array.from({ length: totalSlides }).map((_, idx) => (
              <button
                ...
                className={`h-1.5 sm:h-2 rounded-full transition-all duration-300 ${idx === sliderIndex ? "bg-white w-6 sm:w-8" : "bg-white/50 w-1.5 sm:w-2"}`}
              />
            ))}
          </div>
        </div>
        ...
        {/* Search bar mobile */}
        <div className="block sm:hidden mt-4 relative w-full">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Cari lomba, penyelenggara..." className="w-full pl-12 pr-4 py-2.5 rounded-full bg-card text-foreground border border-border shadow-sm outline-none" />
        </div>
      </section>
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="flex items-center gap-2 text-foreground text-lg font-semibold">
  ...
  return (
    <div onClick={onClick} className="bg-card rounded-xl border border-border overflow-hidden cursor-pointer hover:shadow-lg hover:-translate-y-0.5 transition-all group">
      <div className="relative overflow-hidden" style={{ height: compact ? 140 : 160 }}>
        ...
        {!c.isFree && <div className="absolute top-3 left-3 bg-white/90 text-foreground text-xs font-medium px-2.5 py-1 rounded-full">Rp {c.registrationFee?.toLocaleString("id-ID") || 0}</div>}
      ...
      <div className="p-3 space-y-2">
        ...
        <div className="flex items-center justify-between pt-1 border-t border-border">
          <div className={`flex items-center gap-1 text-xs ${daysLeft < 0 ? "text-muted-foreground" : isUrgent ? "text-red-600 font-semibold" : isWarning ? "text-amber-600" : "text-muted-foreground"}`}>
